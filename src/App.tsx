...
export default function App() {
    ...
    if (loadedRooms) {
      ...
      loadedRooms = loadedRooms.map((room) => {
        if (room.groupName === "Umroh Reguler 11 Juni 2026 (Madinah Awal)") {
          ...
          if (
            room.hotelName === "Makkah" &&
            room.hotelDetailName !== "Rayhaan Marwa Rotana" &&
            room.hotelDetailName !== "Anjum"
          ) {
            room.hotelDetailName =
              num % 2 === 0 ? "Rayhaan Marwa Rotana" : "Anjum";
            migrated = true;
          } else if (
            room.hotelName === "Madinah" &&
            room.hotelDetailName !== "Al Anshor Golden Tulip" &&
            room.hotelDetailName !== "Maden Rawdah"
          ) {
            room.hotelDetailName =
              num % 2 === 0 ? "Al Anshor Golden Tulip" : "Maden Rawdah";
            migrated = true;
          }
        }
        return room;
  ...
  const [hotelInfos, setHotelInfos] = useState<HotelInfographic[]>(() => {
    ...
    return rawList.map((item) => {
      ...
      if (
        restaurantPhoto.startsWith("data:image/") &&
        restaurantPhoto.length > 250000
      ) {
        restaurantPhoto =
          "https://images.unsplash.com/photo-1550966871-3ed3cdb5ed0c?auto=format&fit=crop&q=80&w=800";
      }
      if (
        receptionistPhoto.startsWith("data:image/") &&
        receptionistPhoto.length > 250000
      ) {
        receptionistPhoto =
  ...
  const [checkedPresensiItems, setCheckedPresensiItems] = useState<
  ...
  const [editBioStatus, setEditBioStatus] = useState<
    "Aktif" | "Standby" | "Cuti"
  >("Aktif");
  ...
  const [activeResourceTab, setActiveResourceTab] = useState<
    "sop" | "packages" | "documents"
  >("sop");
  const [handlingReportSubTab, setHandlingReportSubTab] = useState<
    "expenses" | "attendance" | "incident"
  >("expenses");
  const [managerReportSubTab, setManagerReportSubTab] = useState<
    "cashflow" | "schedule"
  >("cashflow");
  ...
  const [msgPriority, setMsgPriority] = useState<"High" | "Medium" | "Low">(
    "Medium",
  );
  ...
  useEffect(() => {
    ...
    const nextRooms = rooms.map((room) => {
      ...
      // Compare arrays
      const isDifferent =
        room.jamaahNames.length !== expectedNames.length ||
      ...
      if (isDifferent) {
        ...
        return {
          ...
          roomType: (expectedNames.length === 2
            ? "Double"
            : expectedNames.length === 4
              ? "Quad"
              : "Triple") as any,
        };
      }
    ...
    Object.keys(combos).forEach((key) => {
      ...
      const expectedNames = combos[key];
      const hasMStyleRoom = nextRooms.some(
        (r) =>
          r.groupName === gName &&
          r.hotelName === "Makkah" &&
          r.id.endsWith(`-${rNum}`),
        ...
        const type =
          expectedNames.length === 2
            ...
            : expectedNames.length === 4
              ...
              : "Triple";
        const mHotel =
          gName.includes("VIP") || gName.includes("Sapphire")
            ? "Fairmont Clock Tower"
            : "Pullman ZamZam Makkah";
        const dHotel =
          gName.includes("VIP") || gName.includes("Sapphire")
            ? "Oberoi Madinah"
            : "Dallah Taibah Madinah";
  ...
  const handleUpdateSelfBiodata = (updatedFields: {
    name: string;
    phone: string;
    username: string;
    password?: string;
    ...
    const idx = teamMembers.findIndex(
      (t) =>
        t.name.toLowerCase() === userLower ||
        t.username.toLowerCase() === userLower,
    );
    if (idx !== -1) {
      ...
      setBioSuccessMsg(
        "✓ Biodata berhasil diperbarui dan tersinkronisasi ke database tim!",
      );
  ...
  const handleAddExpense = (
    newExpense: Omit<
      FieldExpenseReport,
      "id" | "status" | "handlingName" | "handlingId"
    >,
  ) => {
    const userLower = (currentUser || "").toLowerCase();
    const activeTeam = teamMembers.find(
      (t) => t.name.toLowerCase() === userLower,
    );
    ...
    if (activeTeam) {
      ...
      if (
        userLower.includes("fathur") ||
        userLower.includes("yusuf") ||
  ...
  const handleToggleTaskStatus = (id: string) => {
    setDutyTasks((prev) =>
      prev.map((task) => {
        if (task.id === id) {
          let nextStatus: "Belum Selesai" | "Sedang Berjalan" | "Selesai" =
            "Selesai";
  ...
  const handleAddTransaction = (
    newTx: Omit<CashflowTransaction, "id" | "status" | "byUser">,
  ) => {
  ...
  const handleUpdateTransaction = (
    id: string,
    updatedTx: Partial<CashflowTransaction>,
  ) => {
    ...
    setWallets((prev) =>
      prev.map((w) => {
        ...
        const nextAmount =
          updatedTx.amountSAR !== undefined
            ? updatedTx.amountSAR
            : originalTx.amountSAR;
  ...
  const handleTransferFunds = (
    ...
    // Find custom names
    const fromName =
      fromWalletId === "wallet-manager" ? "Kas Pusat (Manager)" : fromWalletId;
  ...
  const handleApproveFieldReport = (reportId: string, walletId: string) => {
    ...
    setExpenses((prev) =>
      prev.map((r) => (r.id === reportId ? { ...r, status: "Selesai" } : r)),
    );
  ...
  const handleRejectFieldReport = (reportId: string) => {
    setExpenses((prev) =>
      prev.map((r) => (r.id === reportId ? { ...r, status: "Ditolak" } : r)),
    );
  };
  ...
  const handleBroadcastMessage = (e: React.FormEvent) => {
    ...
    alert(
      "Pesan instruksi disiarkan secara instant ke seluruh perangkat Handling!",
    );
  };
      ...
      navigator.serviceWorker.ready.then((reg) => {
        if (
          ...
          reg.showNotification(finalTitle, {
            ...
            badge:
              "https://lh3.googleusercontent.com/d/1ADaHuVjVHr8tP1WuWy1q6f8bLGdFYU9a=w400",
          });
          return;
        }
      });
  ...
  const activeDutiesCount = dutyTasks.filter(
    (t) =>
      t.handlingName.toLowerCase() === (currentUser || "").toLowerCase() &&
      t.status !== "Selesai",
  ).length;

  // Filter broadcasts based on currentUser recipients
  const visibleBroadcasts = broadcasts.filter((msg) => {
    if (currentRole === "MANAGER") return true; // Manager sees everything
    if (!msg.recipients || msg.recipients.length === 0) return true;
    const loggedInName = (currentUser || "").toLowerCase();
    const cleanRecipients = msg.recipients.map((r) => r.toLowerCase().trim());
    return (
      cleanRecipients.includes("semua") ||
      cleanRecipients.includes(loggedInName)
    );
  });

  const unreadMessagesCount = visibleBroadcasts.filter((b) => !b.isRead).length;

  // Render Login state first
  if (!currentUser || !currentRole) {
    return (
      <Login onLoginSuccess={handleLoginSuccess} teamMembers={teamMembers} />
    );
  }
  ...
  return (
    <div
      className="min-h-screen bg-slate-900 flex items-center justify-center p-0 sm:p-4 font-sans relative"
      id="applet-viewport"
    >
      {/* Mobile-centric frame mockup centering container */}
      <div
        className="w-full max-w-md h-screen sm:h-[840px] bg-slate-50 flex flex-col relative overflow-hidden sm:rounded-2xl sm:shadow-2xl border-x border-slate-200 pb-16"
        id="mobile-only-viewport"
      >
        {/* GLOBAL TOP NAV HEADER BAR (SIMPLIFIED ROW + DYNAMIC DATE BENEATH) */}
        <header
          className="bg-white border-b border-slate-200 shadow-xs px-4 py-3 shrink-0 z-30 flex flex-col"
        ...
        >
          ...
          <div className="flex items-center justify-between w-full h-8">
            <div className="flex items-center gap-2">
              ...
              />
              <h1 className="text-xs sm:text-sm font-black text-slate-900 tracking-tight uppercase">
        ...
        {refreshMsg && (
          <div className="absolute top-16 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-amber-400 text-[10px] uppercase tracking-wide px-3 py-1.5 rounded-full font-black flex items-center gap-1.5 shadow-lg border border-amber-500/20 animate-in fade-in slide-in-from-top-3 duration-250">
            <span
              className={`w-1.5 h-1.5 rounded-full bg-amber-400 ${isRefreshing ? "animate-ping" : "animate-pulse"}`}
            ></span>
        ...
        >
            ...
            <div className="absolute inset-0 z-50 bg-slate-900/60 flex">
              <div className="w-64 bg-white text-zinc-800 flex flex-col p-4 space-y-4 animate-in slide-in-from-left duration-200 border-r border-[#D4AF37]/20 shadow-xl">
                <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                  <span className="font-serif font-black text-[#D4AF37] tracking-tight uppercase text-xs">
                    JEJAK IMANI
                  </span>
                  <button
                ...
                {/* Navigation list */}
                <nav
                  className="flex-1 space-y-1.5 font-semibold"
                  id="nav-mobile"
                >
                  <button
                    onClick={() => {
                    ...
                    className={`w-full py-2 px-3 rounded text-xs font-bold text-left flex items-center gap-2.5 transition-all ${
                      activeTab === "dashboard"
                        ? "bg-[#D4AF37]/15 border-l-2 border-[#D4AF37] text-slate-900 font-extrabold"
          ...
          <div
            className="bg-white border-b border-slate-200 px-6 py-2.5 hidden items-center justify-between z-10 shrink-0"
            id="top-desktop-app-tabs"
          >
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-slate-700">
                AKSES PORTAL QUICK SWITCH :
              </span>
              <div className="flex gap-1">
                <button
                  onClick={() => {
                    setCurrentRole("HANDLING");
                    handleLoginSuccess(
                      teamMembers[0]?.name || "Tim Lapangan",
                      "HANDLING",
                    );
                  }}
                ...
                <button
                  onClick={() => {
                    ...
                    handleLoginSuccess("Pak Fathur", "MANAGER");
                  }}
                  className={`px-3 py-1 text-[11px] font-extrabold rounded-full border transition-all cursor-pointer ${
                    currentRole === "MANAGER"
            ...
            <div className="flex items-center gap-4 text-xs font-bold text-slate-500">
              ...
              <span>
                Riyal Convert:{" "}
                <strong className="text-emerald-700">1 SAR = Rp 4.350</strong>
              </span>
          ...
          <main
            className="flex-1 overflow-y-auto p-4 sm:p-3 md:p-6 lg:p-8 pb-20 lg:pb-8 space-y-6"
            id="applet-content-scroller"
          >
            {/* GROUP FILTER SELECTOR BANNER FOR JAMAAH RELATED MENUS */}
            {["packages", "documents"].includes(activeTab) && (
              <div
                className="bg-white p-4 rounded-xl border border-slate-250 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in duration-200"
                id="global-group-selector"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block">
                      FILTER DATA KOORDINASI GRUP
                    </span>
                    <h3 className="text-slate-900 text-sm font-bold">
                      Memantau data khusus untuk pilihan rombongan grup berikut:
                    </h3>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {currentRole === "HANDLING" ? (
                    ...
                    >
                      <div className="border-b border-slate-100 pb-3 mb-4 flex justify-between items-center flex-wrap gap-2">
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">
                            Pantau kepatuhan penugasan tim di 3 kota utama Arab
                            Saudi
                          </p>
                        </div>
                        <span className="px-2.5 py-1 bg-[#0F172A] text-[#D4AF37] text-[10px] font-bold rounded border border-[#D4AF37]/20">
                          LIVE MANAGER DECK
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        >
                          <span className="text-[10px] font-black text-indigo-700 block uppercase">
                            Jadwal Aktif
                          </span>
                          <span className="text-xl font-bold text-indigo-950 font-mono tracking-tight">
                            {dutyTasks.length} Tugas
                          </span>
                        ...
                        >
                          ...
                          <span className="text-xl font-bold text-amber-950 font-mono tracking-tight">
                            {rooms.length} Terdistribusi
                          </span>
                          <span className="text-[10px] text-slate-450 block mt-0.5 font-bold">
                            Periksa Manifes →
                          </span>
                      ...
                      <div className="mt-5 space-y-2">
                        <span className="text-[10px] font-bold text-slate-400 block uppercase">
                          PENGAJUAN KEUANGAN DAHSBOARD SENTRAL (LATEST)
                        </span>
                        {expenses.slice(0, 2).map((expense) => (
                          <div
                            key={expense.id}
                            className="p-2.5 rounded bg-slate-50 border border-slate-200 flex items-center justify-between text-xs font-semibold"
                          >
                            <div className="flex items-center gap-2">
                ...
                <div className="lg:col-span-4 space-y-6">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
                    ...
                    <button
                      onClick={() =>
                        setActiveTab(
                          currentRole === "HANDLING" ? "sop" : "dashboard",
                        )
                      }
                  ...
                  <div className="bg-[#0F172A] text-white p-5 rounded-xl border border-slate-800 shadow-md">
                    ...
                    <div className="space-y-3.5" id="dashboard-alerts-hub">
                      {visibleBroadcasts.slice(0, 2).map((msg) => (
                        <div
                          key={msg.id}
                          className="p-3 bg-black/40 rounded-lg border border-slate-800 space-y-1.5"
                        >
                          <div className="flex justify-between items-center">
                            <span
                              className={`text-[9px] px-1.5 py-0.5 rounded font-black tracking-widest ${
                                msg.priority === "High"
                ...
                {activeResourceTab === "packages" && (
                  <div className="space-y-6 animate-in fade-in duration-150">
                    <div
                      ...
                      id="resource-packages-grid"
                    >
                      {packages
                        .filter((p) => p.groupName === selectedGroupFilter)
                        .map((pkg) => (
                          <div
                            key={pkg.id}
                            ...
                            <div>
                              ...
                              <div className="p-5 space-y-4 text-xs">
                                <div className="grid grid-cols-2 gap-4 pb-4 border-b border-slate-100">
                                  <div className="space-y-1">
                                    <span className="text-[10px] text-slate-400 block uppercase font-bold text-emerald-600">
                                      🛫 Keberangkatan
                                    </span>
                                    ...
                                    <div className="font-bold text-slate-800 uppercase">
                                      {pkg.departureFlightCode} •{" "}
                                      {pkg.departureFlightRoute}
                                    </div>
                                    <div className="text-slate-500 font-mono text-[10px]">
                                      {pkg.departureTimeRange}
                                    </div>
                                  </div>
                                  <div className="space-y-1">
                                    <span className="text-[10px] text-slate-400 block uppercase font-bold text-orange-600">
                                      🛬 Kepulangan
                                    </span>
                                    <div className="font-bold text-slate-800">
                                      {pkg.returnDate}
                                    ...
                                    <div className="font-bold text-slate-800 uppercase">
                                      {pkg.returnFlightCode} •{" "}
                                      {pkg.returnFlightRoute}
                                    </div>
                                    <div className="text-slate-500 font-mono text-[10px]">
                                      {pkg.returnTimeRange}
                                    </div>
                                          ...
                                          {pkg.connectedHotels.map((hId) => {
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                Info Hotel {hotelInfo.hotelName}
                                              </button>
                                            );
                                          })}
                                        </div>
                                ...
                                <div className="pb-2">
                                  ...
                                  <div className="grid grid-cols-2 gap-2 bg-amber-50/50 p-2 rounded-lg border border-amber-100/50">
                                    <div>
                                      <span className="text-[10px] text-amber-700/70 block uppercase font-bold">
                                        Kedatangan
                                      </span>
                                      <span className="text-slate-800 font-bold line-clamp-2">
                                        {pkg.arrivalMeals || "-"}
                                      </span>
                                    </div>
                                    <div>
                              ...
                              <div className="p-4 bg-slate-50 border-t border-slate-150 flex justify-end">
                                <button
                                  onClick={() => {
                                    alert(
                                      `Menghubungi Muthawif ${pkg.mutawwifName} via WhatsApp...`,
                                    );
                                  }}
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-4 sm:p-5 space-y-4 animate-in fade-in duration-150">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 p-3 rounded-lg border border-slate-250">
                      ...
                      <div className="text-right text-[10px] text-slate-500 font-bold hidden sm:block">
                        * Seluruh berkas digital tersinkronisasi aman dengan
                        imigrasi Saudi
                      </div>
                    </div>
                        ...
                        <tbody className="divide-y divide-slate-150">
                          {documents
                            .filter(
                              (docGroup) =>
                                docGroupFilter === "All" ||
                                docGroup.groupName === docGroupFilter,
                            )
                            .flatMap((docGroup) =>
                            ...
                            .map((file, idx) => (
                              <tr
                                key={idx}
                                className="hover:bg-slate-55/75 transition-colors font-medium text-slate-800"
                              >
                                <td className="p-3 font-bold text-[#A47F17]">
                                  {file.groupName}
                                </td>
                                ...
                                <td className="p-3">
                                  <div className="flex items-center justify-center gap-2">
                                    <button
                                      onClick={() =>
                                        alert(
                                          `Simulasi pratinjau dokumen ${file.name} : berkas aman dan terverifikasi di imigrasi Riyadh.`,
                                        )
                                      }
                                    ...
                                    <button
                                      onClick={() =>
                                        alert(
                                          `Mengunduh file ${file.name} ke galeri handphone Anda.`,
                                        )
                                      }
                          ...
                          <button
                            ...
                            className={`flex-1 px-4 py-1.5 rounded transition-all cursor-pointer whitespace-nowrap flex items-center justify-center gap-1.5 ${
                              handlingReportSubTab === "expenses"
                                ? "bg-[#0F172A] text-[#D4AF37] font-extrabold"
                  ...
                  <div className="space-y-6">
                        ...
                        <FieldReport
                          ...
                          walletBalance={(() => {
                            ...
                            if (activeTeam) {
                              ...
                              return (
                                wallets.find((w) => w.id === wId)?.balanceSAR ||
                                0
                              );
                            }
                            // Fallback to legacy
                            ...
                            if (
                              ...
                              computedWalletId = "wallet-manager";
                            return (
                              wallets.find((w) => w.id === computedWalletId)
                                ?.balanceSAR || 0
                            );
                          })()}
                          currentUser={currentUser || ""}
                    ...
                    {handlingReportSubTab === "attendance" && (
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-150">
                        ...
                        {(() => {
                          const userLower = (currentUser || "").toLowerCase();
                          const userDuties = dutyTasks.filter(
                            (item) =>
                              item.handlingName.toLowerCase() === userLower &&
                              item.status !== "Selesai",
                          );
                            ...
                            return (
                              <div className="lg:col-span-12 bg-amber-50 border border-amber-200 p-5 rounded-xl text-center space-y-2">
                                ...
                                <p className="text-[11px] text-slate-650 max-w-md mx-auto">
                                  Anda tidak memiliki jadwal penugasan aktif
                                  saat ini, sehingga tidak dapat melakukan
                                  presensi.
                                </p>
                              </div>
                            );
                          ...
                          return (
                            <form
                              onSubmit={(e) => {
                                ...
                                const selectedDutyId = formData.get(
                                  "selected_duty",
                                ) as string;
                                const currentDuty = userDuties.find(
                                  (d) => d.id === selectedDutyId,
                                );
                                const currentCategory = currentDuty
                                  ? currentDuty.roleTag
                                  : "";
                                const currentSop =
                                  taskChecklists[currentCategory] || [];

                                // Build list of completed checklists
                                const completedSopItems = currentSop.map(
                                  (item, idx) => ({
                                    task: item,
                                ...
                                if (currentDuty) {
                                  ...
                                  handleUpdateTask(currentDuty.id, {
                                    status: newStatus,
                                  });
                                }
                                ...
                                alert(
                                  "Presensi berhasil dikirim dan diverifikasi!",
                                );
                              }}
                              className="lg:col-span-5 bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4"
                            >
                              <h3 className="font-extrabold text-slate-900 text-xs uppercase border-b pb-2 tracking-tight">
                                INPUT PRESENSI PENUGASAN
                              </h3>
                              ...
                              <div className="space-y-1">
                                ...
                                <select
                                  ...
                                  required
                                  value={
                                    presensiDutyId || userDuties[0]?.id || ""
                                  }
                                  onChange={(e) => {
                              ...
                              <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase font-black tracking-wider font-sans">
                                  LOKASI LINGKUP PENUGASAN
                                </label>
                                <select
                                  name="location"
                                  required
                                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-semibold text-slate-800 cursor-pointer"
                                >
                                  <option value="Hotel Makkah">
                                    Hotel Makkah
                                  </option>
                                  <option value="Hotel Madinah">
                                    Hotel Madinah
                                  </option>
                                  <option value="Bandara Jeddah">
                                    Bandara Jeddah
                                  </option>
                                  <option value="Bandara Madinah">
                                    Bandara Madinah
                                  </option>
                              ...
                              <div className="space-y-1">
                                ...
                                >
                                  <option value="Masuk Tugas">
                                    Masuk Tugas (Check In)
                                  </option>
                                  <option value="Selesai Tugas">
                                    Selesai Tugas (Check Out)
                                  </option>
                                </select>
                              </div>
                              ...
                              {(() => {
                                const selectedId =
                                  presensiDutyId || userDuties[0]?.id;
                                const activeDuty = userDuties.find(
                                  (d) => d.id === selectedId,
                                );
                                const activeCategory = activeDuty
                                  ? activeDuty.roleTag
                                  : "";
                                const activeSopItems =
                                  taskChecklists[activeCategory] || [];

                                if (
                                  presensiStatus === "Masuk Tugas" &&
                                  activeSopItems.length > 0
                                ) {
                                  return (
                                    <div className="p-3.5 bg-[#D4AF37]/5 border border-[#D4AF37]/20 rounded-lg space-y-2.5">
                                      ...
                                      <div className="space-y-2">
                                        {activeSopItems.map((item, idx) => (
                                          <label
                                            key={idx}
                                            className="flex items-start gap-2 text-[11px] text-slate-700 font-bold cursor-pointer hover:text-slate-900"
                                          >
                                            <input
                                              type="checkbox"
                                              checked={
                                                !!checkedPresensiItems[idx]
                                              }
                                              onChange={(e) => {
                                                setCheckedPresensiItems(
                                                  (prev) => ({
                                      ...
                                      <p className="text-[9px] text-slate-400 italic">
                                        Harap periksa dan centang kegiatan yang
                        ...
                        <div className="lg:col-span-7 bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-3">
                          ...
                          <div
                            className="divide-y divide-slate-100 max-h-96 overflow-y-auto pr-2"
                            id="attendance-feed"
                          >
                            {(() => {
                              ...
                              if (myLogs.length === 0) {
                                return (
                                  <p className="py-8 text-center text-slate-405 italic text-xs">
                                    Belum ada riwayat presensi tercatat untuk
                                    Anda.
                                  </p>
                                );
                              }
                              return myLogs.map((log: any) => (
                                <div
                                  key={log.id}
                                  className="py-3 flex flex-col gap-2.5 text-xs font-semibold"
                                >
                                  <div className="flex justify-between items-center gap-3">
                                    <div className="flex items-start gap-2">
                                      <User className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                                      <div>
                                        <strong className="text-slate-850 font-extrabold">
                                          {log.name}
                                        </strong>
                                  ...
                                  {log.sopItems && log.sopItems.length > 0 && (
                                    <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] space-y-1 text-slate-600 block pl-3.5">
                                      <p className="font-extrabold text-[#A47F17] flex items-center gap-1.5 uppercase tracking-wide text-[10px]">
                                        ...
                                        <span>
                                          SOP Checklist Realisasi Kegiatan:
                                        </span>
                                      </p>
                                      <ul className="space-y-1 font-bold">
                                        {log.sopItems.map(
                                          (sop: any, sIdx: number) => (
                                            ...
                                            >
                                              <span
                                                className={`shrink-0 mt-0.5 text-xs font-black ${sop.completed ? "text-emerald-600" : "text-rose-500"}`}
                                              >
                                                {sop.completed ? "✓" : "✗"}
                                              </span>
                                              <span
                    ...
                    {handlingReportSubTab === "incident" && (
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-150">
                        ...
                        >
                          ...
                          <p className="text-[10.5px] text-slate-400 font-medium leading-normal -mt-2">
                            Isian kalimat pendek (Contoh: Waktu Kedatangan
                            Bandara)
                          </p>
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              name="severity"
                              required
                              className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-805 cursor-pointer"
                            >
                        ...
                        <div className="lg:col-span-7 bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
                          ...
                          >
                            {incidentLogs.map((inc: any) => (
                              <div
                                key={inc.id}
                                className="p-3 bg-slate-50/70 border border-slate-150 rounded-lg space-y-2 text-xs font-semibold"
                              >
                                <div className="flex justify-between items-start flex-wrap gap-2">
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${
                                        inc.severity === "Masukan"
                                          ? "bg-amber-50 text-amber-805 border border-amber-200"
                                          : inc.severity === "Informasi"
            ...
            {activeTab === "messages_disabled" && (
              <div
                className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200"
                id="messages-tab-section"
              >
                ...
                <div className="lg:col-span-7 space-y-4">
                  <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-xs">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Daftar siaran berita urgent dan koordinasi tim dari
                      kordinator di Indonesia
                    </p>
                  </div>
                    ...
                    {broadcasts.map((msg) => (
                      ...
                      >
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 text-[10px] text-slate-400">
                          * Klik pesan ini untuk menandai sudah dibaca di
                          database pusat
                        </div>
                      </div>
                    ))}
                ...
                <div className="lg:col-span-5 bg-white p-5 rounded-xl border border-slate-200 shadow-xs h-fit">
                  <div className="border-b border-slate-100 pb-3 mb-4">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Gunakan modul ini untuk mensimulasikan pengرسalan
                      instruksi dadakan dari Jakarta langsung ke handphone
                      Ahmad/Faiz di tanah suci.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleBroadcastMessage} className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="High">
                          🚨 High Priority (Instant push popup)
                        </option>
                        <option value="Medium">
                          ⚠️ Medium Priority (Standard Newsfeed)
                        </option>
                        <option value="Low">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-700 mb-1">
                        ISI PESAN DETAIL / ALUR INSTALALASI
                      </label>
                      <textarea
                        rows={4}
                        required
                  ...
                  <div className="mt-4 p-3 bg-amber-50 rounded border border-amber-200 text-[11px] text-amber-900 leading-normal flex items-start gap-1.5 font-semibold">
                    ...
                    <span>
                      Simulasi: Setelah mengklik tombol kirim, silakan ganti
                      role ke Lapangan (Ahmad) menggunakan toggler di atas
                      halaman untuk menguji unread notification counter.
                    </span>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "account" &&
              (() => {
                const userLower = (currentUser || "").toLowerCase();
                const matchingMemberIndex = teamMembers.findIndex(
                  (t) =>
                    t.name.toLowerCase() === userLower ||
                    t.username.toLowerCase() === userLower,
                );
                const matchingMember =
                  matchingMemberIndex !== -1
                    ? teamMembers[matchingMemberIndex]
                    : null;
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                            ...
                            <span className="text-xs font-black text-slate-700 uppercase tracking-wide">
                              Edit Biodata Akun Terintegrasi
                            </span>
                          </div>
                          ...
                          <div className="space-y-2.5 text-xs">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                value={editBioPhone}
                                onChange={(e) =>
                                  setEditBioPhone(e.target.value)
                                }
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setEditBioUsername(e.target.value)
                                }
                            ...
                            <div>
                              ...
                              <select
                                ...
                                value={editBioStatus}
                                onChange={(e) =>
                                  setEditBioStatus(e.target.value as any)
                                }
                              >
                            ...
                            <div>
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditBioPassword(e.target.value)
                                  }
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    setShowBioPassword(!showBioPassword)
                                  }
        ...
        <div
          className="absolute bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 z-40 flex justify-around items-center px-1 shadow-lg shrink-0"
        ...
        {isMessagesPopupOpen && (
          <div className="absolute inset-0 z-50 bg-slate-900/65 backdrop-blur-xs flex items-center justify-center p-3 animate-in fade-in duration-150">
            <div
              className="bg-white w-full max-w-sm rounded-xl shadow-2xl border border-slate-200 flex flex-col max-h-[75%] overflow-hidden animate-in zoom-in-95 duration-150"
              id="messages-popup-dialog"
            >
              {/* Header */}
              ...
              <div className="p-4 overflow-y-auto space-y-3 flex-1 bg-slate-50">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 flex flex-col gap-1.5 shrink-0 text-left">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-amber-900 text-[10.5px] uppercase tracking-wide">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!("Notification" in window)) {
                          alert(
                            "Perangkat / browser Anda belum mendukung notifikasi sistem.",
                          );
                          return;
                        }
                        ...
                        if (status === "granted") {
                          alert(
                            "Notifikasi berhasil diaktifkan! Anda akan menerima update live tugas & siaran.",
                          );
                          triggerBrowserNotification(
                            "Notifikasi Berhasil Diaktifkan!",
                            "Sinkronisasi notifikasi real-time aktif.",
                          );
                        } else {
                          alert(
                            "Izin ditolak. Harap aktifkan izin Notifikasi untuk browser Anda melalui pengaturan sistem HP.",
                          );
                        }
                      }}
                  ...
                  <p className="text-[9.5px] text-amber-700 leading-normal font-medium">
                    Pastikan Anda mengizinkan notifikasi agar instruksi penting
                    dari Jakarta langsung masuk sebagai banner pop-up di layar
                    HP Anda.
                  </p>
                </div>
                ...
                {visibleBroadcasts.length === 0 ? (
                  ...
                  visibleBroadcasts.map((msg) => (
                    ...
                    >
                      ...
                      <h4 className="font-extrabold text-slate-905 text-xs leading-snug mt-1">
                        {msg.title}
                      </h4>
                      <p className="text-[11px] text-slate-650 leading-relaxed mt-1.5">
                        {msg.text}
                      </p>
                      {!msg.isRead && (
