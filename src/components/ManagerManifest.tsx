...
export default function ManagerManifest({ 
  ...
  return (
    <div className="space-y-4" id="manager-manifest-com">
      {/* Upper header */}
      <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center bg-white p-4 rounded-xl border border-slate-200 gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {selectedGroupDirectory === null && (
            <div className="flex items-center bg-slate-100 rounded-lg p-0.5 border border-slate-200">
              <button
                ...
                className={`py-1.5 px-3 rounded-md text-[10px] font-black uppercase transition-all ${
                  viewMode === 'group' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Grup
              </button>
              <button
                onClick={() => setViewMode('jamaah')}
                className={`py-1.5 px-3 rounded-md text-[10px] font-black uppercase transition-all ${
                  viewMode === 'jamaah' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Jamaah
              </button>
          ...
          <button
            ...
            className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-lg text-xs font-bold border border-slate-200 transition-all text-center"
          >
            + Buat Grup Baru
          </button>
          <button
            onClick={() => setIsModalOpen(true)}
            className="px-3 py-1.5 bg-slate-900 border border-[#D4AF37]/25 hover:bg-black text-[#D4AF37] rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5"
          >
      ...
      {selectedGroupDirectory === null ? (
        <>
          {viewMode === 'group' && (
            <div className="space-y-4" id="group-directory-grid">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3 shadow-xs">
                <div>
                  <label className="block text-[10px] uppercase font-black text-slate-400 mb-1">Cari Nama / ID Jamaah</label>
                  <div className="relative">
                    <Search className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-slate-400" />
                    <input
                      type="text"
                      placeholder="Ketik nama atau ID jamaah untuk mencari..."
                ...
                <div>
                  <label className="block text-[10px] uppercase font-black text-slate-400 mb-1">Cari Nama Grup Jamaah</label>
                  <div className="relative">
                    <Filter className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-slate-400" />
                    <input
                      type="text"
                      placeholder="Ketik nama grup untuk mencari..."
          ...
          {viewMode === 'jamaah' && (
            <div className="space-y-4" id="jamaah-directory-table">
