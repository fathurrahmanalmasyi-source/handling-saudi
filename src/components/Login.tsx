...
import { ShieldCheck, User, Lock, ArrowRight, BookOpen, KeyRound, Eye, EyeOff } from 'lucide-react';
        ...
        <div className="w-full max-w-sm bg-white rounded-xl shadow-md border border-[#D4AF37]/15 p-5 sm:p-6 relative">
            ...
            <div className="grid grid-cols-2 gap-1.5 p-1 bg-slate-100 rounded-lg">
              <button
                ...
                onClick={() => {
                  ...
                  if (username === 'yusuf') { setUsername(teamList[0]?.name || ''); }
                }}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (username !== 'yusuf') { setUsername('yusuf'); }
                }}
                className={`py-1.5 text-xs font-extrabold rounded-md transition-all cursor-pointer ${
                  loginRole === 'MANAGER'
                    ? 'bg-slate-900 text-[#D4AF37] shadow-xs'
          ...
          <form onSubmit={handleLogin} className="space-y-3.5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                {loginRole === 'HANDLING' && teamList.length > 0 ? (
                  <select
                    value={username}
                    onChange={(e) => { setUsername(e.target.value); setError(''); }}
                    className="w-full py-2 pl-9 pr-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-[#D4AF37] appearance-none"
                  >
                    <option value="" disabled>-- Pilih Nama Tim --</option>
                    <optgroup label="Jeddah">
                       {teamList.filter(t => t.sector === 'Handling Jeddah').map(t => (
                           <option key={t.id} value={t.name}>{t.name}</option>
                       ))}
                    </optgroup>
                    <optgroup label="Madinah">
                       {teamList.filter(t => t.sector === 'Handling Madinah').map(t => (
                           <option key={t.id} value={t.name}>{t.name}</option>
                       ))}
                    </optgroup>
                    <optgroup label="Makkah">
                       {teamList.filter(t => t.sector === 'Handling Makkah').map(t => (
                           <option key={t.id} value={t.name}>{t.name}</option>
                       ))}
                    </optgroup>
                  </select>
                  ...
                  <input
                    ...
                    className="w-full py-2 pl-9 pr-3 bg-slate-50 border border-slate-200 rounded-lg text-xs placeholder:text-slate-400 text-slate-900 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]"
                  />
                )}
              </div>
            </div>
          ...
          <div className="mt-6 pt-5 border-t border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                onClick={() => selectQuickLogin('yusuf', 'MANAGER')}
                className="p-2 border border-dashed border-slate-200 rounded-lg hover:bg-[#D4AF37]/5 text-left transition-all cursor-pointer"
              >
                <div className="text-[11px] font-bold text-slate-800 flex items-center gap-1">
